...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative flex flex-col items-center justify-center min-h-[80vh] mb-20 overflow-hidden">
      {/* Background gradient */}
      <div className="absolute inset-0 bg-gradient-to-br from-blue-900/30 to-purple-900/30 rounded-xl"></div>
      ...
      {isMounted && (
        <>
          ...
          <div className="absolute bottom-10 right-10 w-40 h-40 bg-purple-500/10 rounded-full filter blur-xl animate-pulse animation-delay-2000"></div>
          <div className="absolute top-1/2 left-1/3 w-24 h-24 bg-cyan-500/10 rounded-full filter blur-xl animate-pulse animation-delay-4000"></div>
        </>
      )}
      ...
      <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
        <div className={`transition-all duration-1000 transform ${isMounted ? 'translate-y-0 opacity-100' : 'translate-y-0 opacity-100'}`}>
          <h1 className="text-6xl md:text-7xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-white to-blue-200">
            Hello, I'm <span className="text-blue-500 font-extrabold">Your Name</span>
          </h1>
        </div>
        ...
        <div className={`flex flex-col md:flex-row gap-4 justify-center transition-all duration-1000 transform ${isMounted ? 'delay-600' : ''}`}>
          ...
          <a 
            href="#contact" 
            className="px-8 py-4 bg-transparent hover:bg-white/10 border-2 border-white text-white rounded-full font-medium transition-all duration-300 backdrop-blur-sm hover:shadow-lg"
          >
            Contact Me
          </a>
        </div>
